                            ...
                            <tbody>
                                <tr v-for="(post, index) in posts" :key="index">
                                    ...
                                    <td class="text-center">
                                        <router-link :to="{name: 'post.edit', params:{kode: post.kode }}" class="btn btn-sm btn-primary mr-1">EDIT</router-link>
                                        <button @click.prevent="postDelete(post.kode)" class="btn btn-sm btn-danger ml-1">DELETE</button>
                                    </td>
                                </tr>
                            </tbody>
                        </table>
...
export default {
    setup() {
        ...
        //method delete
        function postDelete(kode) {
            
            //delete data post by ID
            let url = `http://localhost/Penjadwalan/Penjadwalan-Backend/public/api/jam/${kode}`
            if (window.confirm("Are you sure you want to delete this data?")){
                axios.delete(url)
                .then(() => {
              
              //splice posts 
              posts.value.splice(posts.value.indexOf(kode), 1);
